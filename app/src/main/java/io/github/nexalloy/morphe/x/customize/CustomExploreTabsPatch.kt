...
/** Hides tabs from the Explore section (Trending, News, Sports…). */
val CustomExploreTabs = patch(
    name = "Customize explore tabs",
    description = "Hides tabs from the Explore section.",
    use = false,
) {
    dependsOn(XVersionCheck)

    ExploreTabsBuilderFingerprint.hookMethod {
        after { param ->
            val hidden = XPref.getStringSet("x_hide_explore_tabs")
